...
_Generated at 2026-09-25T09:32:58.396616+00:00_ — all three states are evaluated on the same `data/eval/test_set.json`.
...
| Judge Accuracy | 1.0000 | 0.6000 | 1.0000 | -0.4000 | +0.0000 |
...
| --- | --- | --- | --- |
| 1 | drop_latest_records | 5 | Dropped the 5 most recently published records (20%). |
| 2 | blank_summary | 3 | Replaced the abstract with an empty string. |
| 3 | inject_noise | 3 | Inserted garbage tokens (e.g. '#@!%', '0xDEADBEEF', U+FFFD) into the abstract. |
...
- **Retrieval Hit Rate**: 1.0000 → 0.8000 (-20.0% relative change) → 1.0000 after repair (fully recovered).
- **Mean Token F1**: 1.0000 → 0.6568 (-34.3% relative change) → 1.0000 after repair (fully recovered).
- **Judge Accuracy**: 1.0000 → 0.6000 (-40.0% relative change) → 1.0000 after repair (fully recovered).
- **Mean Judge Score (1-5)**: 5.0000 → 3.4000 (-32.0% relative change) → 5.0000 after repair (fully recovered).
- Largest token-F1 drop by question type: `date` (−0.5000).
- The GX gate flagged the corrupted batch with 3 failed expectation(s): `expect_column_values_to_be_unique(paper_id)`, `expect_column_value_lengths_to_be_between(title)`, `expect_column_value_lengths_to_be_between(summary)`.
- Freshness SLA on corrupted data: stale ratio 31.82% → `is_fresh=False`; latest paper moved to 2026-06-12 because the newest records were dropped.
- Repaired data re-passed the gate (`success=True`, `is_fresh=True`).
- Idempotency: repaired dataset fingerprint `c1257bb4d221` matches the baseline fingerprint `c1257bb4d221`; a second rebuild from raw gave the same fingerprint.

### Why this is a silent failure

On the corrupted index the QA path returned an answer for 10/10 questions and raised no error, yet 5 answers were wrong or degraded. Nothing in the serving path signalled a problem; only the GX gate and the freshness SLA did.
...
Repair never patches the corrupted table. It rebuilds the clean dataset from the immutable raw snapshot (`data/raw/crossref_records.json`) with the same cleaning code, re-validates it with the GX gate and re-indexes it into a separate `papers-repaired` collection. Because the input and the transformation are both deterministic, running the repair any number of times yields the same dataset.
