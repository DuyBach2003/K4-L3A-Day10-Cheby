...
def build_test_set(df: pd.DataFrame, output_path) -> list[dict[str, Any]]:
    """Build a fixed benchmark from the clean dataframe and write it to `output_path`.

    Papers are picked at evenly spaced positions of the recency-sorted corpus (newest,
    oldest and everything in between), so the benchmark is deterministic and not
    cherry-picked. Titles containing a single quote are skipped because the question
    template quotes the title.
    """
