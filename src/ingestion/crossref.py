...
def _authors(item: dict[str, Any]) -> list[str]:
    ...
    for author in item.get("author") or []:
        ...
        full_name = full_name or normalize_whitespace(str(author.get("name", "")))
        if full_name:
...
def parse_crossref_payload(payload: dict) -> list[PaperRecord]:
    ...
    Records without DOI, title, abstract or a parseable publication date are dropped,
    and repeated DOIs keep only their first occurrence.
    """
    ...
    for item in items:
        ...
        categories = [strip_markup(str(subject)) for subject in item.get("subject") or [] if str(subject).strip()]
...
def fetch_source_records(settings: Settings) -> list[PaperRecord]:
    ...
    - Offline/dev mode (default): read the snapshot at `raw_api_response`.
    - Live mode (`REFRESH_SOURCE=1`, or no snapshot yet): call the Crossref API with retry on
      429/5xx; if the API stays unavailable, fall back to the snapshot when one exists.
    """
    ...
    if settings.refresh_source or not raw_path.exists():
        try:
            ...
            if parse_crossref_payload(live_payload):
                ...
                print("[ingestion] Live Crossref response had no usable records; using snapshot.")
...
def load_raw_records(path: Path) -> list[PaperRecord]:
    """Read the parsed raw snapshot (`crossref_records.json`) back into `PaperRecord`s."""
