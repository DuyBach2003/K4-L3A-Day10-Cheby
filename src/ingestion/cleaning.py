...
def build_clean_dataframe(records: list[PaperRecord], run_date: datetime) -> pd.DataFrame:
    ...
    Rows without id/title/summary or a parseable `published` date are dropped; `age_days`
    is measured against `run_date` so the freshness SLA can be evaluated downstream.
    """
    today = run_date.date()
    ...
    for record in records:
        ...
        authors = [strip_markup(author) for author in record.authors]
        categories = [strip_markup(category) for category in record.categories]
        rows.append(
            {
                "paper_id": paper_id,
                "title": title,
                ...
                "author_count": len([author for author in authors if author]),
