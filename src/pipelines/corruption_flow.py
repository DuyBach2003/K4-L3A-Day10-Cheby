...
from pipelines.phase1 import QualityGateError, dataset_fingerprint, load_dataframe, save_dataframe
from retrieval.index import LocalEmbeddingIndex


def repair_from_raw(settings: Settings) -> pd.DataFrame:
    """Idempotent repair: rebuild the clean dataset from the immutable raw snapshot, never patch in place."""
...
def main() -> None:
    ...
    # 2) Observability on the corrupted batch
    ...
    # 3) Measure the silent failure: in production the gate would block this batch; here it is
    #    indexed into an isolated collection on purpose to quantify the damage.
    ...
    # 4) Auto-repair when the gate trips: rebuild from raw, twice, to prove idempotency.
    if gate_tripped:
