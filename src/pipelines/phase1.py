...
def _run_agent_demo(settings: Settings, index: LocalEmbeddingIndex, test_set: list[dict[str, Any]]) -> None:
    ...
    try:
        ...
        payload["answers"] = [
            {"question": item["question"], "ground_truth": item["ground_truth"], "agent_answer": run_agent_question(agent, item["question"])}
    ...
    except Exception as exc:  # the demo is optional; missing API keys must not fail the pipeline
...
def main() -> None:
    ...
    print(
        f"[phase1] GX quality gate success={quality['success']} "
